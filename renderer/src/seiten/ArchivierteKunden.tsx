...
export default function ArchivierteKunden() {
  ...
  useEffect(() => {
    ;(async () => {
      const list = await window.db?.archivKundenList?.()
      setKunden(list || [])
    })()
  }, [])
  ...
  const { isInGruppe, settings } = useTableSettings('kunden', keys)
  const gefiltert = useMemo(() => {
    const q = query.trim().toLowerCase()
    if (!q) return sorted
    const vorCol = sorted.length ? Object.keys(sorted[0]).find(k => /vor|vname|vorname/i.test(k)) : null
    const nachCol = sorted.length ? Object.keys(sorted[0]).find(k => /nach|fname|familien|nachname|fam\. Nam/i.test(k)) : null
    ...
    <Layout>
      <div style={{ display: 'grid', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <h2 style={{ margin: 0 }}>Archivierte Kunden</h2>
          ...
          {!!gefiltert.length && (
            <ArchivDropdownZeilen
              ...
              makeTitle={(row)=> {
                ...
                const vorCol = keys.find(k => /vor|vname|vorname/i.test(k))
                const nachCol = keys.find(k => /nach|fname|familien|nachname|fam\. Nam/i.test(k))
...
function DebugBox() {
  ...
  useEffect(() => { (async()=> setDbg(await window.db?.archivDebug?.()))() }, [])
  if (!dbg) return null
  return (
